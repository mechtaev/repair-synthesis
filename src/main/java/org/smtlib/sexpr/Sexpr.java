...
/** This is the tf class for S-expressions in the concrete syntax */
public abstract class Sexpr extends Pos.Posable implements ISexpr {
	...
	public static class Seq extends Sexpr implements ISeq {
		...
		public String getValue(IPrinter p) {
		  ISexpr pair = this.sexprs().get(0);
          String value = p.toString(((ISexpr.ISeq)pair).sexprs().get(1));
          return value;
		}
	...
	public static class Token<T> extends Sexpr implements IToken<T> {
        ...
        public String getValue(IPrinter p) {
          ...
          return value.toString(); //Seryozha: a fix to be able to transform :pattern annotation values
        }
