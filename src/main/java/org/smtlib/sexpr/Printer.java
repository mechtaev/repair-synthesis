...
public class Printer implements IPrinter, org.smtlib.IVisitor</*@Nullable*/ Void> {
    ...
    } else if (e instanceof ISexpr.IToken) { //Seryozha: a fix to be able to transform :pattern annotation values
	...
	public Void visit(ISexpr.IToken<?> e) throws IVisitor.VisitorException {
		try {
        w.append(e.getValue(this)); //Seryozha: a fix to be able to transform :pattern annotation values
		} catch (IOException ex) { throw new IVisitor.VisitorException(ex); }
		return null;
	}
